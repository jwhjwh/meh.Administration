...
#warning 必须先打开数据库才能创建表  否则提示数据库没有打开
    [_fmdb executeUpdate:@"CREATE TABLE IF NOT EXISTS t_userpas(id INTEGER PRIMARY KEY, userid TEXT NOT NULL, password TEXT NOT NULL,isopen INTEGER NOT NULL)"];
}

+ (BOOL)insertModel:(Userpas *)model {
    NSString *insertSql =  [NSString stringWithFormat:@"INSERT INTO t_userpas(userid, password,isopen) VALUES ('%@', '%@','%d')",model.userid,model.password,model.isopen];
...
+ (NSArray *)queryData:(NSString *)querySql {
    ...
    while ([set next]) {
        ...
        int  isopen = [[set stringForColumn:@"isopen"]intValue];
