...
@implementation ZXDChineseString
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
        NSString *pinyin = [((ZXDChineseString*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((ZXDChineseString*)object).pinYin substringToIndex:1];
        NSString *string = ((ZXDChineseString*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    {
        ZXDChineseString *chineseString = [[ZXDChineseString alloc]init];
        ...
        chineseString.string = [ZXDChineseString RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        [result addObject:((ZXDChineseString*)[tempArray objectAtIndex:i]).string];
        NSLog(@"SortArray----->%@",((ZXDChineseString*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
