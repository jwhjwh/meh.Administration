...
-(void)gotoAddTrack
{
    VCAddTrack *vc = [[VCAddTrack alloc]init];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [ShareModel shareModel].storeName = dict[@"storeName"];
    if ([dict[@"code"]intValue]==1) {
        [self.navigationController pushViewController:vc animated:YES];
    }else
    {
        [ELNAlerTool showAlertMassgeWithController:self andMessage:@"暂无权限" andInterval:1.0f];
        return;
    }
}
