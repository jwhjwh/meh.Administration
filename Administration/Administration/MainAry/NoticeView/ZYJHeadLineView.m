...
//        self.hidenTwoLabel.text = hienModel.title;
    ...
    if (flag == 0) {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            self.hidenView.frame = CGRectMake(0, 10, self.frame.size.width, self.frame.size.height);
        } completion:^(BOOL finished) {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            self.currentView.frame = CGRectMake(0, 10, self.frame.size.width, self.frame.size.height);
        } completion:^(BOOL finished) {
...
- (void)createCurrentView
{
    ...
    self.currentView = [[UIView alloc]initWithFrame:CGRectMake(0, 10, self.frame.size.width, self.frame.size.height)];
...
//    
    //内容标题
    self.currentLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 3, self.currentView.frame.size.width, self.currentView.frame.size.height-30)];
    self.currentLabel.text = model.title;
...
//    [self.hidenView addSubview:self.hidenBtn];
    ...
    //内容标题
    self.hidenLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 3, self.hidenView.frame.size.width, self.hidenView.frame.size.height-30)];
