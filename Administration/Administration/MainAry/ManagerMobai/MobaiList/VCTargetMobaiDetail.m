...
@interface VCTargetMobaiDetail ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
@implementation VCTargetMobaiDetail
...
- (void)viewDidLoad {
    ...
    _nameArrs = @[@[@"签到记录"],@[@"日期",@"拜访时间段",@"拜访次数"],@[@"店名",@"地址",@"负责人",@"职务",@"固话/手机",@"微信号"],@[@"店规模",@"床位",@"美容师人数",@"开店年限",@"主要经营品牌",@"关注品牌"],@[@"目前了解信息:"],@[@"店家问题简述",@"品牌介入策略及跟进规划",@"店家要求事项及解决办法",@"同事协助须知"],@[@"店家预合作时间:",@"执行方案:",@"合约金额:"],@[@"打款方式"]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section <4){
        ...
        if (indexPath.section ==0) {
            ...
            UILabel *qdlabel = [[UILabel alloc]initWithFrame:CGRectMake(((self.view.frame.size.width/2)-30)+30, 10, 100, 30)];
...
-(void)selectTargetVisit{
    ...
    dict = @{
             ...
             @"TargetVisitId":self.OldTargetVisitId
             };
        [ZXDNetworking GET:urlStr parameters:dict success:^(id responseObject) {
            if ([[responseObject valueForKey:@"status"]isEqualToString:@"0000"]) {
                ...
                NSDictionary *dictinfo = [responseObject valueForKey:@"recordInfo"][0];
                
                    TargetModel *model=[[TargetModel alloc]init];
                    [model setValuesForKeysWithDictionary:dictinfo];
                    [_InterNameAry addObject:model];
                    _state = [NSString stringWithFormat:@"%@",[dictinfo valueForKey:@"state"]];
                    NSLog(@"0-----%@",_state);
                    _dipatement = [NSString stringWithFormat:@"%@",[dictinfo valueForKey:@"departmentId"]];
                    _UserId = [NSString stringWithFormat:@"%@",[dictinfo valueForKey:@"userId"]];
                    [self nstingallocinit:model];
                    
                    _nameArrs = @[@[@"签到记录"],@[@"拜访日期",@"拜访时间段",@"拜访次数"],@[@"店名",@"地址",@"负责人",@"职务",@"固话/手机",@"微信号"],@[@"店规模",@"床位",@"美容师人数",@"开店年限",@"主要经营品牌",@"关注品牌"],@[@"目前了解信息:"],@[@"店家问题简述",@"品牌介入策略及跟进规划",@"店家要求事项及解决办法",@"同事协助须知"],@[@"店家预合作时间:",@"执行方案:",@"合约金额:"],@[@"打款方式"]];
